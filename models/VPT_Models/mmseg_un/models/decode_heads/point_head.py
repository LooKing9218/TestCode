...
from VPT_Models.mmseg_un.models.builder import HEADS
from VPT_Models.mmseg_un.ops import resize
from VPT_Models.mmseg_un.models.losses import accuracy
from VPT_Models.mmseg_un.models.decode_heads.cascade_decode_head import BaseCascadeDecodeHead
...
class PointHead(BaseCascadeDecodeHead):
    ...
    def get_points_train(self, seg_logits, uncertainty_func, cfg):
        ...
        shift = num_sampled * torch.arange(
            batch_size, dtype=torch.long).cuda()
        idx += shift[:, None]
        point_coords = point_coords.view(-1, 2)[idx.view(-1), :].view(
            batch_size, num_uncertain_points, 2)
        if num_random_points > 0:
            rand_point_coords = torch.rand(
                batch_size, num_random_points, 2).cuda()
